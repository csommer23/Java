...
public class BST {
    ...
    **/
    ...
    public int countNodes() {    	
    	if(isEmpty()) return 0;   	
    	return countNodes(getRootNode(),1)-1;
    }    
    
    protected int countNodes(BSTNode p,int count) {
    	if (p != null) count = countNodes(p.left,1) + countNodes(p.right,1);   		
    	return count;
    }
    
    public int countLeafNodes() {
    	if(isEmpty()) return 0;   	
    	return countLeafNodes(getRootNode(),1);
    }
    
    protected int countLeafNodes(BSTNode p,int count) {
    	if (p != null) {  		
    		if(p.left == null && p.right == null) return 1;    		
    		count = countLeafNodes(p.left,0) + countLeafNodes(p.right,0); 
    	}
    	return count;
    }
    ...
    protected int height(BSTNode p,int count) {
    	if (p != null) count += height(p.left,1);    	
    	return count;
    }
    
    public boolean isFull() {
    	return ((int)Math.pow(2,height())-1) == countNodes() ? true : false;
    }
}
